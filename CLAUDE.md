...
本仓库是用户的学习项目。**项目代码必须由用户亲手写**，Claude 不要代写。这是为了肌肉记忆、面试表达和真正的能力沉淀。

### 不要做

- 不要直接 Edit / Write 用户项目里的源文件（`dev-knowledge-agent/app/**` 及后续主项目源码）
- 不要替用户"顺手"创建脚本、模块、测试文件
- 不要先动手再问，看到任务就开干

### 应该做

- 讲清楚思路：为什么这么写、整体结构、关键 API 的作用
- 给**示例片段**而不是完整文件，关键行让用户自己组装
- 让用户写完后贴出来或让 Claude 读文件 review
- 用户明确说"帮我写"或"你来改"时，再动手
- 文档类文件（`docs/notes/**`、`README.md`、笔记 / 复盘）可以由 Claude 起草，用户审阅修改

### 如果忍不住动了手

立刻停下，告诉用户改了哪些文件，请用户决定是 `git checkout` 还原、还是保留参考。不要继续往下推进。
